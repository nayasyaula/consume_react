...
export default function TrashUser() {
  ...
  const restoreUser = (id) => {
    instance.put(`user/restore/${id}`, null, {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("access_token"),
        },
      })
        ...
        <div className="items-center m-5 pb-10 pt-10">
          <div className="flex justify-between">
            ...
            <div className="flex justify-end">
              <Link to='/user' className="px-4 py-2 bg-teal-700 text-white shadow-md border-sky-500 rounded-lg">Back to User</Link>
            </div>
          </div>
                ...
                {trashs.map((user, id) => (
                  <tr key={user.id} className="border-b dark:border-neutral-500">
                    ...
                    <td className="whitespace-nowrap px-6 py-4">
                      <button type="button" onClick={() => restoreUser(user.id)} className="px-4 py-2 bg-orange-500 rounded-lg mr-2 font-bold text-white">
                        Restore
                      </button>
                      <button type="button" onClick={() => deletePermanentUser(user.id)} className="px-4 py-2 bg-red-500 rounded-lg mr-2 font-bold text-white">
                        Hapus Permanent
                      </button>
                    </td>
                  </tr>
